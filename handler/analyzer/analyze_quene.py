...
class AnalyzeQueneHandler(BaseShellHandler):
    def __init__(self, context):
        super(AnalyzeQueneHandler, self).__init__()
    ...
    def init_option(self):
        ...
        quenue_option = Util.get_option(options, 'quenue')
        if tenant_option:
            ...
            if len(sql_result) <= 0:
                ...
                return False
            self.stdio.verbose("sql_result is ".format(sql_result))
            self.tenant_id = sql_result["tenant_id"]
            self.ip_list = sql_result["ip_list"]
        self.quenue = quenue_option
        if files_option:
            self.is_ssh = False
